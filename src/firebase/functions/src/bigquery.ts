 ...
 * @param datasetId the expected BigQuery dataset ID
 * @param tableId the expected BigQuery table ID
 * @returns true if the table or dataset is missing, false otherwise
 */
export const isMissingTableError = function (bigqueryError: any) {
    if (typeof bigqueryError !== 'object' || bigqueryError === null || !('errors' in bigqueryError)) {
        return false;
    }

    const errorrs = Array.isArray(bigqueryError.errors) ? (bigqueryError.errors as Array<any>) : [];
 ...
 * @param datasetId the expected BigQuery dataset ID
 * @param tableId the expected BigQuery table ID
 * @param body the request body of the incoming API request representing the event
 */
